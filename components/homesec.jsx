...
import { Play, Plus, ThumbsUp, ThumbsDown, Copy, Send, CheckCircle2, Circle, Menu, X } from "lucide-react";

/* ── DATA ─────────────────────────────────────────────────────── */

const HEADLINES = [
  { line1: "You handle the business.", line2: "We convert messages into revenue." },
  { line1: "AI agents that reply instantly,", line2: "sell continuously, 24/7." },
  { line1: "Every unread message", line2: "is a lost customer." },
];
...
function ChatPanel({ chatIndex, setChatIndex, className = "" }) {
  ...
  return (
    <div className={`bg-[#EFEFEF] flex flex-col p-5 h-full ${className}`}>
      ...
      <div className="bg-white flex items-center gap-3 p-2.5 mt-1 shadow-sm border border-gray-200">
        ...
        <button className="bg-gray-100 p-1.5 rounded-sm text-gray-600 hover:bg-gray-200 transition-colors">
          <Send size={14} />
        </button>
      </div>
    </div>
...
export default function FaigenGridHero() {
  ...
  useEffect(() => {
    const t = setInterval(() => setHlIndex(p => (p + 1) % HEADLINES.length), 3200);
    ...
    <section className="min-h-screen bg-white font-sans text-[#1A1A1A] flex flex-col selection:bg-blue-100">
      ...
      <header className="flex justify-between items-center px-6 md:px-12 py-6 max-w-[1400px] mx-auto w-full z-50">
        ...
        <nav className="hidden lg:flex items-center gap-8 text-[15px] font-medium text-gray-800">
          ...
          {["Product","Use Cases","Pricing","Blog","About"].map(n => (
            <a key={n} href="#" className="hover:text-black transition-colors">{n}</a>
          ))}
        </nav>

        <div className="hidden lg:flex items-center gap-3">
          <button className="text-gray-600 text-[14px] font-semibold px-4 py-2.5 hover:text-black transition-colors">Sign in</button>
          <button className="bg-gray-100 text-gray-900 px-5 py-2.5 text-[14px] font-semibold hover:bg-gray-200 transition-colors">Try it for free</button>
        </div>
      ...
      <AnimatePresence>
        {menuOpen && (
          <motion.div
            ...
            className="lg:hidden bg-white border-b border-gray-200 px-6 pb-5 flex flex-col z-40"
          >
            {["Home","Product","Use Cases","Pricing","Blog","About"].map(n => (
              <a key={n} href="#" className="text-[15px] font-medium text-gray-700 py-3 border-b border-gray-100 last:border-0">{n}</a>
            ))}
            <button className="mt-4 bg-gray-100 text-gray-900 py-3 text-[14px] font-semibold hover:bg-gray-200 transition-colors">
              Try it for free
            </button>
          </motion.div>
        )}
      ...
      <main className="flex flex-col items-center justify-center pt-16 pb-14 px-6 relative z-10 w-full max-w-[1200px] mx-auto text-center flex-1">
        ...
        <div className="relative w-full flex flex-col items-center mb-6" style={{ minHeight: "clamp(120px, 21vw, 210px)" }}>
          <AnimatePresence mode="wait">
            <motion.h1
              key={hlIndex} variants={textV} initial="enter" animate="center" exit="exit"
              className="absolute top-0 left-0 right-0 text-[2.6rem] sm:text-[3.8rem] md:text-[5.5rem] lg:text-[6.5rem] font-medium leading-[1.0] tracking-tight text-[#1A1A1A]"
            >
        ...
        <div className="flex items-center gap-2 mb-7">
          {HEADLINES.map((_, i) => (
            <button key={i} onClick={() => setHlIndex(i)}
              className={`transition-all duration-300 rounded-full ${i === hlIndex ? "w-5 h-2 bg-[#2563EB]" : "w-2 h-2 bg-gray-300 hover:bg-gray-400"}`}
              aria-label={`Headline ${i + 1}`}
            />
          ))}
        </div>

        <div className="relative w-full flex justify-center" style={{ minHeight: "60px" }}>
          <AnimatePresence mode="wait">
            <motion.p key={hlIndex} variants={subV} initial="enter" animate="center" exit="exit"
              className="absolute text-[1rem] md:text-[1.2rem] text-gray-600 font-normal max-w-[600px] leading-relaxed px-2 md:px-0">
              {SUBTEXTS[hlIndex]}
            </motion.p>
          </AnimatePresence>
        </div>
        ...
        >
          <button className="bg-[#2563EB] text-white px-7 py-3 rounded-md font-semibold text-[15px] hover:bg-blue-700 transition-all shadow-sm">
            Try it for free
          </button>
          <button className="bg-white text-gray-800 border border-gray-200 px-7 py-3 rounded-md font-semibold text-[15px] hover:bg-gray-50 transition-colors flex items-center gap-2 shadow-sm">
      ...
      <div className="lg:hidden w-full border-t border-gray-200">
        ...
        <div style={{ minHeight: 420 }}>
          <ChatPanel chatIndex={chatIndex} setChatIndex={setChatIndex} />
        </div>
      ...
      {/* ══════════════════════════════════════════════════════════
          ...
          row-span-3 chat + exact row heights = perfect alignment
      ══════════════════════════════════════════════════════════ */}
      <div className="hidden lg:block w-full border-t border-gray-200 mt-8">
        <div className="min-w-[1200px] grid grid-cols-5 w-full bg-white">
          ...
          {/* CENTER CHAT — row-span-3, height = 128+192+128 = 448px + 2px borders */}
