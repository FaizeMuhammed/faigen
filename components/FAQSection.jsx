...
import { Plus, X, Mail, ShieldCheck, Check, Copy } from "lucide-react";
          ...
          <div className="p-7 md:p-8">
            <div className="flex justify-between items-start mb-6">
              ...
              <button onClick={onClose}
                className="text-[#86868B] hover:text-[#1D1D1F] bg-[#F5F5F7] border border-[#E5E5EA] w-8 h-8 flex items-center justify-center rounded-full">
                <X size={16} strokeWidth={2} />
              </button>
            </div>
            <h3 className="text-[20px] font-semibold text-[#1D1D1F] tracking-tight mb-2">Contact our team</h3>
            <p className="text-[14px] text-[#86868B] leading-relaxed mb-7">
              Have a specific question not covered in the FAQ? Reach out to our deployment team.
            </p>
            <div className="bg-[#F5F5F7] border border-[#E5E5EA] rounded-2xl p-4 flex items-center justify-between group hover:border-[#D2D2D7] hover:bg-[#EAEAEB] transition-all">
              <div className="flex items-center gap-3 md:gap-4">
                <Mail size={18} className="text-[#86868B] shrink-0" />
                <a href="mailto:info@faigen.in" className="text-[14px] font-medium text-[#1D1D1F]">info@faigen.in</a>
              </div>
              <button onClick={handleCopy}
                className="text-[#86868B] hover:text-[#0066CC] p-2 bg-white shadow-sm rounded-xl border border-[#E5E5EA] shrink-0">
...
const FAQItem = ({ faq, isOpen, onToggle }) => (
  <div
    className={`w-full overflow-hidden transition-all duration-300 rounded-[20px] md:rounded-[24px] cursor-pointer border ${
      isOpen
        ? "bg-white border-[#E5E5EA] shadow-[0_8px_30px_rgb(0,0,0,0.04)] z-10 relative"
        : "bg-transparent border-[#E5E5EA] hover:bg-[#F5F5F7] relative"
    }`}
    onClick={onToggle}
  >
    <div className="flex justify-between items-center p-5 md:p-8">
      <h4 className={`text-[15px] md:text-[18px] font-semibold tracking-tight pr-4 md:pr-6 transition-colors leading-snug ${isOpen ? "text-[#1D1D1F]" : "text-[#86868B]"}`}>
        {faq.question}
      </h4>
      <div className={`flex-shrink-0 w-9 h-9 md:w-10 md:h-10 rounded-full flex items-center justify-center transition-all duration-300 ${isOpen ? "bg-[#1D1D1F] text-white shadow-md rotate-180" : "bg-white border border-[#E5E5EA] text-[#86868B]"}`}>
        {isOpen ? <X size={16} strokeWidth={2.5} /> : <Plus size={16} strokeWidth={2} />}
      </div>
    </div>
    <AnimatePresence initial={false}>
      {isOpen && (
        <motion.div
          ...
          transition={{ duration: 0.4, ease: [0.16, 1, 0.3, 1] }}
        >
          <div className="px-5 md:px-8 pb-6 md:pb-8 text-[14px] md:text-[15px] font-medium text-[#86868B] leading-relaxed">
            {faq.answer}
          </div>
        </motion.div>
      )}
...
export default function FAQSection() {
  ...
  return (
    <section className="relative w-full py-14 md:py-32 font-sans text-[#1D1D1F] bg-[#FBFBFD] overflow-hidden selection:bg-[#0066CC] selection:text-white">
      ...
      <div className="max-w-[1500px] mx-auto px-5 md:px-12 xl:px-16 relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 md:gap-12 lg:gap-20 items-start">
          ...
          >
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white border border-[#E5E5EA] text-[#86868B] text-[11px] font-bold tracking-widest uppercase shadow-[0_2px_8px_rgba(0,0,0,0.02)] mb-6 md:mb-8">
              <span className="w-2 h-2 rounded-full bg-[#0066CC] animate-pulse" />
              Support & Queries
            </div>

            <h2 className="text-[2.4rem] sm:text-[3.2rem] md:text-[4.5rem] font-semibold leading-[1.02] tracking-tighter text-[#1D1D1F] mb-5 md:mb-6">
              Frequently <br className="hidden lg:block" />
              asked <br className="hidden lg:block" />
              questions.
            </h2>
            <p className="text-[#86868B] text-[15px] md:text-[18px] font-medium mb-8 md:mb-12 max-w-full lg:max-w-[420px] leading-relaxed tracking-tight">
              Find answers to frequently asked questions about our AI agents, integrations, and deployment timelines.
            </p>

            <button
              onClick={() => setPopupOpen(true)}
              className="bg-[#1D1D1F] text-white px-6 md:px-8 py-3.5 md:py-4 rounded-full text-[14px] md:text-[15px] font-semibold shadow-[0_8px_20px_rgba(0,0,0,0.08)] transition-all duration-300 flex items-center gap-2 group hover:bg-black w-full sm:w-auto justify-center sm:justify-start"
            >
              Contact our team
              <Mail size={15} className="text-[#86868B] group-hover:text-white transition-colors" />
            </button>
          </motion.div>
          ...
          <motion.div
            initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-50px" }} variants={fadeUp}
            className="lg:col-span-7 flex flex-col gap-3 md:gap-4 relative"
          >
            {faqs.map((faq) => (
              <FAQItem
                key={faq.id}
