...
export default function FooterSection() {
  ...
  // Link config — href: null means open the popup, href: string means navigate
  const sections = [
    {
      title: "Services",
      links: [
  ...
  return (
    <footer className="w-full bg-[#FBFBFD] pt-2 font-sans text-[#1D1D1F] overflow-hidden">
      ...
      <div className="w-full px-4 md:px-8 mb-20">
        ...
        >
          {/* Arrows removed — just the headline */}
                ...
                {section.links.map((link) => (
                  <li key={link.label}>
                    {link.href ? (
                      ...
                      <button onClick={() => setShowPopup(true)} className="hover:text-[#0066CC] transition-colors">
                        {link.label}
                      </button>
                    )}
                  </li>
